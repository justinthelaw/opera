...
export const optimize = (
    sentence: string,
    evalFcn: (str: string) => Results
): { status: STATUS; rendering: Results } => {
    const smallerSpace = '\u2006'
    const largerSpace = '\u2004'
    ...
    if (
        ...
        (newSpace === largerSpace && worstCaseResults.overflow < STATUS.MAX_UNDERFLOW.valueOf())
    ) {
    ...
    while (finalOptimStatus === STATUS.NOT_OPT) {
        ...
        optWords.splice(indexToReplace, 2, optWords.slice(indexToReplace, indexToReplace + 2).join(newSpace))

        //make all other spaces the normal space size
        const newSentence = optWords.join(' ')

        //console.log(newSentence.split(' '))
        const newResults = evalFcn(newSentence)
        ...
        if (newSpace === largerSpace && newResults.overflow > 0) {
            ...
            finalResults = newResults
            if (newSpace === largerSpace && finalResults.overflow > STATUS.MAX_UNDERFLOW.valueOf()) {
                finalOptimStatus = STATUS.OPTIMIZED
            } else {
...
export const renderBulletText = (text: string, getWidth: (str: string) => number, width: number): Results => {
    ...
    if (fullWidth < width) {
        ...
        if (getWidth(textSplit[0].trimEnd()) < width) {
            ...
            const guessIndex = width / avgCharWidth
