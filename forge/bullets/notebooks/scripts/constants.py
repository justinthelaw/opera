...
# This is the device to which the model will be loaded to
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# This is the regex for a standard bullet pattern "-[ACTION];[IMPACT]--[OUTCOME]"
# The pattern is not all-encompassing - this hardcoded value can to be experimented with to achieve maximum bullet capture
bullet_pattern = r"^-?([\w\W\s]{0,255});?([\w\W\s]{0,255})--([\w\W\s]{0,255})$"

# Input prefix used for fine tuning the model to comprehend as a key phrase for bullet interpretation task
...
# Input prefix used for fine tuning the model to comprehend as a key phrase for bullet creation tasking
